...
try:
	window.iconbitmap('~/quadcopters-tfg-lvaro/Dronekit/favicon.ico')
except:
	pass
...
def changeSelection(*args):
	if str(platformValue.get())=="SITL":
		...
		if str(platformUAVselect.get())=="USB":
			mvpyAddress.set("/dev/ttyACM0")
			mvpyPort.set("115200")
		elif str(platformUAVselect.get())=="Telemetry":
			mvpyAddress.set("/dev/ttyUSB0")
			mvpyPort.set("57600")
		else:
			mvpyAddress.set("")
			mvpyPort.set("")
...
platformUAV.select()	# Selected by default
...
def launchMavproxy(address,port):
	def launchMavproxyThrd(address,port):
		openCMD='lxterminal --command="'
		mavproxyRoute='sudo mavproxy.py '
		if address[0:5]=="/dev/":
		...
		os.system(openCMD + mavproxyRoute + mavproxyArgs + '"')
	thrdLaunchMavproxy=thrd(launchMavproxyThrd,address,port)
	thrdLaunchMavproxy.start()
...
def runScript(route):
	def runScriptThrd(route):
		openCMD='lxterminal --command="'
		scriptRoute='sudo python ~/quadcopters-tfg-lvaro/Dronekit/' + route + '/main.py'
		os.system( scriptRoute )
	thrdRunScript=thrd(runScriptThrd,route)
	thrdRunScript.start()
