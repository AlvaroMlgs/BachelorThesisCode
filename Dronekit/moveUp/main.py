...
altObs=Observe(vehicle.location.global_relative_frame.alt,vehicle.location.global_relative_frame.alt,vehicle.location.global_relative_frame.alt)
def checkAlt(target):
	while not altObs.geq(target):
		altObs.update(vehicle.location.global_relative_frame.alt)
		time.sleep(0.02)
	return altObs.geq(target)

autoMove = Auto(do_move, checkAlt, [altObs.initial+5,1,[0,0,1]], altObs.initial+4.5)
